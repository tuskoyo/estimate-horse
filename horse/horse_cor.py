...
def pose_estimate_from_tracking_data(
    ...
    # Load the model
    pose_model = init_pose_model(config_path, model_path, device=device)
    pose_model.cfg.visualizer.radius = radius
    pose_model.cfg.visualizer.alpha = alpha
    ...
    visualizer.set_dataset_meta(pose_model.dataset_meta, skeleton_style=skeleton_style)
    ...
    while True:
        ...
        pose_results, returned_outputs = inference_top_down_pose_model(
            pose_model, img, [{'bbox': [x1, y1, x2 - x1, y2 - y1]}], format='xywh', dataset='AnimalPose', dataset_info=None, return_heatmap=False
        )
        ...
        position_data = [frame_idx] + pose_results[0]['keypoints'][:, :2].ravel().tolist()
        position_df.loc[len(position_df)] = position_data
        visibility_data = [frame_idx] + pose_results[0]['keypoints'][:, 2].tolist()
...
if __name__ == "__main__":
    pose_estimate_from_tracking_data(
        ...
        model_path="/home/ohwada/human_pose_estimation/models/rtmpose-l_simcc-coco-wholebody_pt-aic-coco_270e-384x288-eaeb96c8_20230125.pth",
        config_path="/home/ohwada/human_pose_estimation/models/rtmpose-l_8xb32-270e_coco-wholebody-384x288.py",
