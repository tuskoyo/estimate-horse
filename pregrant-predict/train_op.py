...
def train_model_loocv(X, y, file_names, processed_dir, fold_pred_dir, device='cuda', epochs=30):
    """
    LOOCVを用いて、各サンプルの時系列データに対するモデルの評価を行い、
    各foldの予測結果CSVを fold_pred_dir に出力する。
    入力:
      - X: 特徴量の時系列データ（リスト）
      - y: 各時間ステップのラベル（リスト）
      - file_names: 各サンプルのファイル名（リスト）
      - processed_dir: 加工済みCSVファイルのディレクトリ（再読み込み用）
      - fold_pred_dir: LOOCVの各fold予測結果CSVの保存先ディレクトリ
      - device: 'cuda' または 'cpu'
      - epochs: 訓練エポック数
    出力:
    ...
    if fold_pred_dir and not os.path.exists(fold_pred_dir):
        os.makedirs(fold_pred_dir)
    ...
    for train_index, test_index in loo.split(X_padded):
        ...
        # 各foldのテストサンプルの予測結果CSVを出力（fold_pred_dirが指定されていれば）
        if fold_pred_dir:
            ...
            try:
                ...
                out_fp = os.path.join(fold_pred_dir, test_file)
...
# ===============================
frame_buffer = []
WINDOW_SIZE = 120  # 例: 1fpsで2分間（120フレーム）

def process_new_frame(frame_features, model):
    """
    新たに1フレーム分の特徴量（numpy配列, shape=(特徴量次元,)）を受け取り、
    グローバルバッファに追加。ウィンドウサイズに達した場合、モデルに入力して予測を実施する。
    出力:
      - 予測結果（確信度）をコンソールに出力し、バッファから最古のフレームを削除する。
    """
    global frame_buffer
    frame_buffer.append(frame_features)
    if len(frame_buffer) >= WINDOW_SIZE:
        window_data = np.array(frame_buffer[-WINDOW_SIZE:])
        window_data = np.expand_dims(window_data, axis=0)  # shape=(1, WINDOW_SIZE, 特徴量次元)
...
# ===============================
def predict_and_save_csv_fold(processed_dir, fold_pred_dir, file_name, y_pred):
    """
    指定されたprocessed_dir内のCSVファイルを再読み込みし、各時間ステップごとの予測結果（pred_label）を
    カラムとして追加して、fold_pred_dirに保存する。
    入力:
      - processed_dir: 加工済みCSVファイルのディレクトリ
      - fold_pred_dir: 予測結果付きCSVファイルの保存先ディレクトリ
      - file_name: 対象CSVのファイル名
      - y_pred: LOOCVで予測されたラベル配列（1次元、各時間ステップの予測）
    出力:
      - 対象ファイルに 'pred_label' カラムを追加してfold_pred_dirに保存する。
    """
    try:
        ...
        out_fp = os.path.join(fold_pred_dir, file_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fold_pred_dir", default="train_output", help="LOOCV各foldの予測結果付きCSVの保存先ディレクトリ")
    ...
    # (B) LOOCVによるモデル評価と、各foldごとに予測結果CSV出力（fold_pred_dirが指定されていれば）
    train_model_loocv(X, y, file_names, processed_dir=args.data_dir, fold_pred_dir=args.fold_pred_dir, device=args.device, epochs=args.epochs)
    ...
    if args.final_model:
        ...
        # こちらは任意で、fold_pred_dirが指定されていれば実行
        if args.fold_pred_dir:
            # 最終モデルで全CSVファイルの予測結果を出力する（既にLOOCVで個別出力済みの場合は不要）
            print("最終モデルを用いた全CSVファイルの予測結果出力を開始します。")
            # 再利用する関数: predict_and_save_csv_fold()（ここでは全CSV出力版）
            for file_name in file_names:
                try:
                    ...
                    out_fp = os.path.join(args.fold_pred_dir, file_name)
                    df.to_csv(out_fp, index=False)
