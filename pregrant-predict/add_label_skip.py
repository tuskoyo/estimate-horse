...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="指定時間の1時間前のデータにラベルを付与するプログラム（指定時間以降のデータは削除）"
    )
    ...
    if args.target_time == "9999":
        # target_timeが9999の場合はすべてのlabelを0に設定（フィルタリングは行わず全行出力）
        df['label'] = 0
    else:
        ...
        # 指定時間以降（target_datetime以降）の行は削除する
        df = df[df.index < target_datetime]

        # 新たなカラム "label" を作成
        # 1時間前の時間範囲内の行は1、それ以外は0
        df['label'] = ((df.index >= start_time) & (df.index < target_datetime)).astype(int)
