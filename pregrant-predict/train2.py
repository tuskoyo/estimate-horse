import os
# GPUメモリ断片化対策のための環境変数設定（プログラム冒頭で設定）
...
def train_model_loocv(X, y, file_names, device='cuda', epochs=30):
    """
    LOOCVを用いて、各サンプルの時系列データに対するモデルの評価を行う。
    入力:
      - X: 特徴量の時系列データ（リスト）
      - y: 各時間ステップのラベル（リスト）
...
# ===============================
frame_buffer = []
WINDOW_SIZE = 120  # 例: 1fpsで4分間（120フレーム）
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="PyTorchを使用した馬の陣痛行動予測モデル（深層学習部分）")
    ...
    def load_processed_data_with_temporal_labels(data_dir, downsample_factor=1):
        ...
        for fp in file_paths:
            try:
                ...
                features_normalized = (features - features_min) / features_range
                if downsample_factor > 1:
                    features_normalized = features_normalized[::downsample_factor]
